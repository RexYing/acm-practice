...
public class PackingDominoes_betterCode {

	int[][] field;
	int K, w, h;
	Domino[] D;

	public PackingDominoes_betterCode(int K, int w, int h) {
		K++;
		this.K = K;
		this.w = w;
		this.h = h;
	...
	boolean isValid(int pips, int x, int y) {
		return matchOrEmpty(pips, x - 1, y) && matchOrEmpty(pips, x + 1, y) && matchOrEmpty(pips, x, y - 1)
	...
	public static void main(String[] args) throws IOException {
		...
		while ((l = in.readLine()) != null) {
			...
			if (K == -1)
				break;
			new PackingDominoes_betterCode(K, w, h);
		}
	}
